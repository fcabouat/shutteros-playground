...
async function openNext(page: Page, id: string) {
...
for (const delivery of ['http', 'file'] as const) {
  test(`${delivery}: complete all situations, inspect spoofing, and reset`, async ({
    ...
    await expect(page.getByText('Créé par François Cabouat · Licence MIT')).toBeVisible();
    ...
    await expect(page.locator('.about-notice').first()).toContainText(
      'Copyright (c) 2026 François Cabouat',
    );
    ...
    await expect(
      page.getByText('Le domaine qui reçoit vos données', { exact: false }),
    ).toBeVisible();
    await page.getByRole('button', { name: 'Revenir à mon favori connu', exact: false }).click();
    await advance(page);
...
test('the desktop USB opens an explorer before running a file triggers the incident', async ({
  ...
  await page.getByRole('button', { name: 'Ouvrir / exécuter', exact: true }).click();
  ...
  await expect(
    page.getByText('L’ouverture de la clé a déclenché une infection simulée.'),
  ).toBeVisible();
});
...
test('native browser and phone keep teaching actions outside the application', async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  await page.goto('./');
  await enter(page);
  ...
  await expect(page.locator('[data-challenge="mfa"] .choice-button')).toHaveCount(0);
...
test('left Start menu opens updates and keeps their status for the session', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Mises à jour', exact: true }).click();
  await expect(
    page.getByRole('button', { name: 'Accepter la mise à jour prévue par l’IT' }),
  ).toBeVisible();
});
