...
describe('portable configuration preparation', () => {
  ...
  it('embeds a named partner logo separately from the primary organisation logo', async () => {
    const root = await fixtureFor({
      version: 1,
      organizationName: 'Primary organisation',
      organizationLogo: 'primary.svg',
      partnerOrganizationName: 'Partner organisation',
      partnerOrganizationLogo: 'partner.svg',
    });
    try {
      await writeFile(resolve(root, 'static/primary.svg'), '<svg></svg>');
      await writeFile(resolve(root, 'static/partner.svg'), '<svg></svg>');
      await run(root);
      const generated = await readFile(
        resolve(root, 'portable/configuration.generated.ts'),
        'utf8',
      ...
      expect(generated).toContain(
        'embeddedOrganizationLogo: string | undefined = "data:image/svg+xml',
      );
      expect(generated).toContain(
        'embeddedPartnerOrganizationLogo: string | undefined = "data:image/svg+xml',
      );
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  ...
  it('rejects a partner logo without a named partner', async () => {
    const root = await fixtureFor({ partnerOrganizationLogo: 'partner.svg' });
    try {
      await expect(run(root)).rejects.toThrow(
        'partnerOrganizationLogo requires partnerOrganizationName',
      );
    } finally {
      await rm(root, { recursive: true, force: true });
    }
