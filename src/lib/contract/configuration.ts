...
export function decodeConfiguration(value: unknown): DecodeConfigurationResult {
  ...
  validateOptionalLogo(value.partnerOrganizationLogo, 'partnerOrganizationLogo', issues);
  if (value.partnerOrganizationLogo !== undefined && value.partnerOrganizationName === undefined) {
    issues.push({
      path: 'partnerOrganizationName',
      message: 'is required with partnerOrganizationLogo',
    });
  }
...
function validateOptionalLogo(value: unknown, path: string, issues: ConfigurationIssue[]): void {
