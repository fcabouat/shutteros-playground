...
<Popover.Root bind:open>
  ...
  <Popover.Portal>
    ...
    >
      ...
      <div class="grid grid-cols-3 gap-2 px-4 py-5">
        {#each challengeOrder as id (id)}
          {@const done = snapshot.results.some((r) => r.id === id)}
          <button
            class="start-app flex flex-col items-center gap-2 rounded-lg p-3 text-center"
      ...
      <div class="border-t border-[var(--line)] px-6 py-4">
        ...
        <button
          ...
          ><Icon name="secure" size={19} /><span class="text-sm">{copy.shell.finish}</span></button
        >
      </div>
      <div
        class="start-profile flex items-center justify-between gap-4 rounded-b-xl border-t border-[var(--line)] px-6 py-4"
      >
