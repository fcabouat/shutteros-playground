...
In the simulation, the desktop icon first opens a fictional file explorer. A single click opens an activity or file; there is no separate file-selection step. Opening the disguised executable or archive triggers a simulated infection. Opening the readme records a risk and shows a warning in the fictional editor, while leaving the activity open. A subsequent safe action earns an amber assessment: the player took a risk, then reacted appropriately. The individual recovery action remains correct. These outcomes describe the simulated journey, not a ranking of file extensions: neither a text filename nor the absence of visible symptoms proves that a real unknown drive is safe.

![USB feedback: a protective action after opening an unchecked file](images/usb-caution.png)
...
In the AI situation, choose an internal or commercial tool, inspect the exercise policy, preview a prepared message, then send it. There is no real login, free-text entry, upload or AI request. Five variants cover routine end-of-year meal planning notes and security notes, each with or without names, plus a request for a general working method. Neutral titles and an interleaved order encourage reading the drafts; the security notes carry only a “[Security]” marker.

The fictional policy permits both versions of the routine project and general methods in the internal tool. The organisation does not ban commercial services, but excludes named, sensitive or recognisable internal information. The permitted routine draft removes names and identifying project details, alongside the general method option. Neither accepts either version of the restricted security notes: removing names does not remove access vulnerabilities or remediation dates. Feedback explains the distinction after sending. Approval of this fictional draft is not a guarantee that removing names adequately anonymises a real document. These exercise rules do not replace the deploying organisation’s policy or AI usage charter.
