...
An interactive terminal offers a keyboard menu unless --keyboard-layout is set.
...
select_keyboard() {
  printf '\nKeyboard for the kiosk:\n  1) Detected: %s (variant: %s)\n  2) French AZERTY\n  3) US QWERTY\n  4) Another XKB layout\n' "$keyboard_layout" "${keyboard_variant:-default}"
  while :; do
    printf 'Choice [1]: '
    IFS= read -r keyboard_choice || die 'keyboard selection cancelled; use --keyboard-layout for automation'
    case "$keyboard_choice" in
      ''|1) break ;;
      2) keyboard_layout=fr; [ "$keyboard_variant_set" = true ] || keyboard_variant=oss; break ;;
      3) keyboard_layout=us; [ "$keyboard_variant_set" = true ] || keyboard_variant=; break ;;
      4)
        printf 'XKB layout (for example be, de, gb): '
        IFS= read -r keyboard_layout || die 'keyboard selection cancelled'
        if [ "$keyboard_variant_set" = false ]; then
          printf 'XKB variant [default]: '
          IFS= read -r keyboard_variant || die 'keyboard selection cancelled'
        fi
        break ;;
      *) printf '%s\n' 'Choose 1, 2, 3 or 4.' ;;
    esac
...
prepare_environment() {
  ...
  detected_layout=$(read_default_assignment "$KEYBOARD_DEFAULTS_FILE" XKBLAYOUT)
  [ -n "$keyboard_layout" ] || keyboard_layout=$detected_layout
  ...
  # Variants belong to a layout: a French host's "oss" is invalid for a US override.
  if [ "$keyboard_layout_set" = true ] && [ "$keyboard_layout" != "$detected_layout" ] && [ "$keyboard_variant_set" = false ]; then
    keyboard_variant=
  fi
  if [ "$keyboard_layout_set" = false ] && [ -t 0 ] && [ -t 1 ]; then
    select_keyboard
  fi
  ...
  printf 'Kiosk keyboard: %s (variant: %s)\n' "$keyboard_layout" "${keyboard_variant:-default}"
...
stop_kiosk_sessions() {
  kiosk_uid=$(id -u "$KIOSK_USER") || return 1
  # PAM and Snap move children outside Cage's service cgroup. Wait for their
  # dedicated user slice to stop before userdel checks for remaining processes.
  loginctl terminate-user "$KIOSK_USER" 2>/dev/null || true
  systemctl stop "user@${kiosk_uid}.service" "user-${kiosk_uid}.slice"
}

backup_support_files() {
  ...
  for pair in \
    ...
    "kiosk.service:$KIOSK_UNIT" \
    "policy.json:$POLICY_FILE"; do
    name=${pair%%:*}
    path=${pair#*:}
    if exists_or_link "$path"; then
      root_owned_regular "$path" || die "refusing to replace unsafe installed file: $path"
      cp -p "$path" "$support_rollback/$name"
    else
      : > "$support_rollback/$name.absent"
    fi
  done
}
...
restore_support_files() {
  ...
  for pair in \
    ...
    name=${pair%%:*}
    path=${pair#*:}
    if [ -f "$support_rollback/$name" ]; then
      install -o root -g root -m "$(stat -c %a "$support_rollback/$name")" "$support_rollback/$name" "$path"
    else
      rm -f "$path"
    fi
  done
...
kiosk_group_created=false
static_group_created=false
...
cleanup() {
  ...
  if [ "$status" -ne 0 ] && [ "$installation_mode" = reinstall ] && [ "$support_mutation_started" = true ]; then
    restore_support_files
  ...
  if [ "$status" -ne 0 ] && [ "$installation_mode" = reinstall ] && [ "$static_account_created" = true ]; then
    userdel "$STATIC_USER" 2>/dev/null ||
      printf '%s\n' 'error: failed to remove the static-server account created by this reinstall' >&2
    if [ "$static_group_created" = true ] && getent group "$STATIC_USER" >/dev/null 2>&1; then
      groupdel "$STATIC_USER" 2>/dev/null ||
        printf '%s\n' 'error: failed to remove the static-server group created by this reinstall' >&2
    fi
  fi
  ...
  if [ "$status" -ne 0 ] && [ "$installation_mode" = fresh ] && [ "$fresh_mutation_started" = true ]; then
    ...
    if [ "$kiosk_account_created" = true ]; then
      # An error stopping user services should not skip identity cleanup. userdel
      # independently refuses removal if the account still owns processes.
      stop_kiosk_sessions || true
      if ! userdel --remove "$KIOSK_USER"; then
        accounts_removed=false
      fi
    fi
    if [ "$static_account_created" = true ] && ! userdel "$STATIC_USER"; then
      accounts_removed=false
    fi
    if [ "$kiosk_group_created" = true ] && getent group "$KIOSK_USER" >/dev/null 2>&1; then
      groupdel "$KIOSK_USER" || accounts_removed=false
    fi
    if [ "$static_group_created" = true ] && getent group "$STATIC_USER" >/dev/null 2>&1; then
      groupdel "$STATIC_USER" || accounts_removed=false
    fi
    if [ "$accounts_removed" = true ]; then
      rm -f "$STATIC_UNIT" "$KIOSK_UNIT" "$POLICY_FILE"
      [ "$kiosk_root_created" = false ] || rm -rf "$KIOSK_ROOT"
      rm -rf "$LIB_ROOT"
    else
      printf '%s\n' 'error: account cleanup failed; retained installation ownership files for administrator recovery' >&2
    fi
    systemctl daemon-reload 2>/dev/null || true
  fi
  [ -z "$support_rollback" ] || rm -rf "$support_rollback"
  exit "$status"
}
trap cleanup EXIT
trap 'exit 129' HUP
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --keyboard-layout) [ "$#" -ge 2 ] || die '--keyboard-layout requires a value'; keyboard_layout=$2; keyboard_layout_set=true; shift 2 ;;
...
if [ "$installation_mode" = fresh ]; then
  ...
  if useradd --create-home --user-group --shell "$LIB_ROOT/session" "$KIOSK_USER"; then
    kiosk_account_created=true
    kiosk_group_created=true
  else
    useradd_status=$?
    # useradd exit 12 means the identity was committed before home creation failed.
    # Other failures can include a concurrent administrator creating the same name.
    if [ "$useradd_status" -eq 12 ]; then
      id "$KIOSK_USER" >/dev/null 2>&1 && kiosk_account_created=true
      getent group "$KIOSK_USER" >/dev/null 2>&1 && kiosk_group_created=true
    fi
    die 'failed to create the kiosk account'
  fi
  write_marker
else
...
if [ "${static_account_missing:-false}" = true ]; then
  if useradd --system --user-group --no-create-home --home-dir /nonexistent --shell /usr/sbin/nologin "$STATIC_USER"; then
    static_account_created=true
    static_group_created=true
  else
    die 'failed to create the static-server account'
  fi
fi

if [ "$installation_mode" = reinstall ]; then
  backup_support_files
